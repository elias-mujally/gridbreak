...
import type { MapId } from '../src/game/modes';
...
type InternalCreate = { code: string; mapId: MapId; playerCount: 2 | 3 | 4; sessionId: string; tokenHash: string; displayName: string; now: number };
...
export class RoomDurableObject extends DurableObject<Env> {
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    ctx.blockConcurrencyWhile(async () => {
      ...
      this.room = stored ? OnlineRoom.restore(stored) : null;
    });
  }
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === '/create' && request.method === 'POST') {
      ...
      await this.persist();
      return json({ room: this.room.snapshot(body.now) }, 201);
    }
    if (url.pathname === '/join' && request.method === 'POST') {
      ...
      await this.persist();
      this.broadcast({ type: 'SNAPSHOT', protocolVersion: PROTOCOL_VERSION, buildVersion: GAME_BUILD_VERSION, sequence: result.value.sequence, room: result.value });
  ...
  async webSocketMessage(ws: WebSocket, raw: ArrayBuffer | string) {
    ...
    if (message.roomCode.toUpperCase() !== this.room.snapshot(Date.now()).code || message.sessionId !== attachment.sessionId) { this.violation(ws, attachment, 'UNAUTHENTICATED', 'Message identity does not match this connection.'); return; }
    if (message.type === 'PING') {
      this.send(ws, { type: 'PONG', protocolVersion: PROTOCOL_VERSION, buildVersion: GAME_BUILD_VERSION, sequence: this.room.snapshot(Date.now()).sequence });
      return;
    }
    ...
    this.broadcast({ type: 'SNAPSHOT', protocolVersion: PROTOCOL_VERSION, buildVersion: GAME_BUILD_VERSION, sequence: result.snapshot.sequence, room: result.snapshot });
  }
  ...
  async alarm() {
    ...
    if (changed) this.broadcast({ type: 'SNAPSHOT', protocolVersion: PROTOCOL_VERSION, buildVersion: GAME_BUILD_VERSION, sequence: snapshot.sequence, room: snapshot });
  ...
  private async authenticateSocket(ws: WebSocket, attachment: SocketAttachment, message: AuthMessage) {
    if (!this.room || message.roomCode.toUpperCase() !== this.room.snapshot(Date.now()).code || !ROOM_CODE_PATTERN.test(message.roomCode.toUpperCase())) { this.violation(ws, attachment, 'ROOM_NOT_FOUND', 'Room not found.'); return; }
  ...
  private violation(ws: WebSocket, attachment: SocketAttachment, code: OnlineErrorCode, message: string) {
    ...
    this.send(ws, rejection(code, message, this.room?.snapshot(Date.now()).sequence ?? 0));
  ...
  private broadcast(event: ServerEvent) {
    for (const socket of this.ctx.getWebSockets()) if ((socket.deserializeAttachment() as SocketAttachment | null)?.sessionId) this.send(socket, event);
  }
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/rooms' && request.method === 'POST') {
      ...
      for (let attempt = 0; attempt < 8; attempt++) {
        ...
        const response = await forwardJson(stub, '/create', { code, mapId: valid.value.mapId, playerCount: valid.value.playerCount, sessionId, tokenHash, displayName: valid.value.displayName, now: Date.now() } satisfies InternalCreate);
