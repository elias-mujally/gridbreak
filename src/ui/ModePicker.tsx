...
import { MAP_CONFIGS, MAP_IDS, type ConvergencePlayerCount, type MapId, type MatchConfiguration, type RaceLayout, compatibleLayouts, mapDimensions, mapSupportsMode, matchCapability, onlineMatchConfiguration, unsupportedMapReason } from '../game/modes';
...
type PlayWith = 'ai' | 'local' | 'online';
export default function ModePicker({ onStart, onOnline }: { onStart: (options: NewGameOptions, difficulty: Difficulty) => void; onOnline: (configuration?: MatchConfiguration) => void }) {
  ...
  const [playWith, setPlayWith] = useState<PlayWith>('ai');
  ...
  const activeLayout = mode === 'convergence' ? 'convergence' : layout;
  const capability = matchCapability(mapId, mode, activeLayout, mode === 'convergence' ? playerCount : 2);
  ...
  function selectPlayWith(value: PlayWith) {
    if (mode === 'rush' && value === 'local') return;
    setPlayWith(value); setError('');
    if (value === 'ai') setControllers(current => current.map((item, index) => index === 0 ? { type: 'HUMAN_LOCAL' } : { type: 'AI', difficulty: item.difficulty ?? 'normal' }));
    if (value === 'local') setControllers(current => current.map((_item, index) => index === 0 ? { type: 'HUMAN_LOCAL' } : { type: 'HUMAN_LOCAL' }));
  }
  function start() {
    if (playWith === 'online') {
      if (!capability?.online) { setError('This configuration is unavailable online.'); return; }
      onOnline(onlineMatchConfiguration(mode, mapId, activeLayout, mode === 'convergence' ? playerCount : 2));
      return;
    }
  ...
  return <div className="mode-screen">
    ...
    <main className="mode-content">
      <div className="eyebrow"><span className="signal-dot" /> MODE → MAP → LAYOUT → PLAY WITH → MATCH SETUP</div>
      ...
      <div className="mode-cards" role="group" aria-label="Game mode">
        <button className={`mode-card ${mode === 'classic' ? 'chosen' : ''}`} onClick={() => selectMode('classic')} aria-pressed={mode === 'classic'}><span className="mode-number">01 / CLASSIC</span><span className="mode-glyph classic-glyph" aria-hidden="true"><i /><i /><i /><i /></span><strong>Pure strategy.</strong><span>Race against AI, a nearby friend, or an online opponent.</span><small>AI / LOCAL / ONLINE</small></button>
        <button className={`mode-card rush-card ${mode === 'rush' ? 'chosen' : ''}`} onClick={() => selectMode('rush')} aria-pressed={mode === 'rush'}><span className="mode-number">02 / RUSH</span><span className="mode-glyph rush-glyph" aria-hidden="true">↗<i /></span><strong>Controlled pressure.</strong><span>Play against AI or take private Phantom information online.</span><small>AI / ONLINE</small></button>
        <button className={`mode-card convergence-card ${mode === 'convergence' ? 'chosen' : ''}`} onClick={() => selectMode('convergence')} aria-pressed={mode === 'convergence'}><span className="mode-number">03 / CONVERGENCE</span><span className="mode-glyph convergence-glyph" aria-hidden="true">◆<i /><i /><i /><i /></span><strong>Race to center.</strong><span>Bring two to four players together through local, AI, or online play.</span><small>AI / LOCAL / ONLINE</small></button>
      </div>
      <button className="online-launch" onClick={() => onOnline()}><span><small>PRIVATE ROOM</small><strong>JOIN BY CODE</strong><em>The host’s authoritative configuration loads automatically.</em></span><b>JOIN ↗</b></button>
      <section className="map-picker" aria-label="Match setup">
        ...
        <div className="map-picker-heading layout-heading"><strong>PLAY WITH</strong><span>Online uses the same map and rules with remote human controllers.</span></div>
        <div className="layout-options play-with-options" role="group" aria-label="Play with"><button className={playWith === 'ai' ? 'chosen' : ''} onClick={() => selectPlayWith('ai')}><strong>AI</strong><span>Local tactical opponent</span></button><button className={playWith === 'local' ? 'chosen' : ''} onClick={() => selectPlayWith('local')} disabled={mode === 'rush'} title={mode === 'rush' ? 'Phantom privacy requires separate devices.' : undefined}><strong>Local Human</strong><span>{mode === 'rush' ? 'Unavailable: private Phantom information' : 'Shared device'}</span></button><button className={playWith === 'online' ? 'chosen' : ''} onClick={() => selectPlayWith('online')} disabled={!capability?.online}><strong>Online Human</strong><span>Private authoritative room</span></button></div>
        {playWith !== 'online' && <><div className="map-picker-heading layout-heading"><strong>CONTROLLERS</strong><span>{mode === 'rush' ? 'Shared-screen Rush stays Human vs AI so Phantom Walls remain secret.' : 'Choose who controls each local slot.'}</span></div>
      ...
      <button className="start-button" onClick={start}>{playWith === 'online' ? 'Create Online Room' : 'Start'} · {map.name} · {mode === 'convergence' ? `${playerCount} Players` : layout === 'parallel' ? 'Parallel' : 'Opposite'} <span>↗</span></button>
    </main>
