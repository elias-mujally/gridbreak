  ...
  return <div className="mode-screen">
    ...
    <main className="mode-content">
      ...
      <section className="map-picker" aria-label="Map selection">
        ...
        <div className="map-options">{mapIds.map(id => { const option = MAP_CONFIGS[id]; return <button key={id} className={mapId === id ? 'chosen' : ''} onClick={() => selectMap(id)} aria-pressed={mapId === id}><strong>{option.name}</strong><span>{option.width}×{option.height}</span></button>; })}</div>
