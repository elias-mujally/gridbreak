...
export default function App() {
  ...
  const moves = useMemo(() => game.turn === 'blue' ? legalMoves(board) : [], [game.turn, board]);
  ...
  const canAct = game.turn === 'blue' && !game.winner;
  const rushMode = game.mode === 'rush';
  const map = MAP_CONFIGS[game.mapId];
  const placementMode = isWallMode(mode);
  const legalHover = hover ? canPlaceViewedWall(view, hover, isPhantomMode(mode)) : false;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (action && next) {
        setGame(next); const event = captureEvent(next);
        setNotice(next.winner ? next.winner === 'draw' ? 'The match ends in a draw.' : next.winner === 'blue' ? 'Victory.' : 'The rival reached your edge.' : event?.text ?? `Rival ${describeAction(action)}. Your turn.`);
      }
      setThinking(false);
    }, 420);
  ...
  function play(action: Action) {
    ...
    const event = captureEvent(next);
    setNotice(next.winner ? next.winner === 'draw' ? 'The match ends in a draw.' : 'You reached the far edge.' : event?.text ?? `You ${describeAction(action)}. Rival is thinking…`);
  }
  ...
  return <div className={`app-shell ${rushMode ? 'rush-shell' : ''} map-${game.mapId} layout-${game.layout} ${game.winner ? 'match-ended' : ''}`}>
    ...
    <main className="main-layout">
      <aside className="intro-panel"><div className="eyebrow"><span className="signal-dot" /> {rushMode ? 'RUSH MODE · LOCAL AI' : 'SINGLE PLAYER · LOCAL AI'}</div><h1>Make<br /><em>your way.</em></h1>
      ...
      <section className="arena" style={{ maxWidth: arenaMax }} aria-label={`${game.width} by ${game.height} GridBreak board`}>
        <div className="arena-top"><div className="player-tag red"><span className="player-orb" /><span className="player-copy"><small>OPPONENT</small><strong>RIVAL</strong>{rushMode && <EnergyMeter player="red" energy={view.rush!.energy.red} assists={view.rush!.assists.red} />}</span></div>
        ...
        <div className="mobile-toolbar" role="group" aria-label="Your action"><button className={mode === 'move' ? 'selected' : ''} onClick={() => { setMode('move'); setHover(null); }} disabled={!canAct}>✣ <span>Move</span></button><button className={mode.endsWith('horizontal') ? 'selected' : ''} onClick={() => selectWall('horizontal')} disabled={!canAct || game.remaining.blue === 0}>━ <span>Wall —</span></button><button className={mode.endsWith('vertical') ? 'selected' : ''} onClick={() => selectWall('vertical')} disabled={!canAct || game.remaining.blue === 0}>┃ <span>Wall |</span></button></div>
      ...
      <aside className="control-panel"><div className="panel-heading"><span className="panel-index">02 / CONTROL ROOM</span><span className="live-pip">LIVE MATCH</span></div><div className="turn-card"><div className="turn-card-top"><span>{rushMode ? `${map?.name.toUpperCase()} / CURRENT TURN` : 'CURRENT TURN'}</span><span className="turn-symbol">{game.winner ? '◆' : '●'}</span></div><strong>{game.winner ? game.winner === 'draw' ? 'Draw' : game.winner === 'blue' ? 'Victory' : 'Defeat' : thinking || game.turn === 'red' ? 'Rival thinking' : 'Your move'}</strong><p>{notice}</p></div>
        ...
        <div className="control-group actions-group"><div className="control-label"><span>YOUR ACTION</span><small>ONE PER TURN</small></div><button className={`action-button ${mode === 'move' ? 'selected' : ''}`} onClick={() => { setMode('move'); setHover(null); }} disabled={!canAct}><span className="action-icon move-icon">✣</span><span><strong>Move</strong><small>Advance one cell</small></span><span className="action-check">{mode === 'move' ? '●' : '○'}</span></button><div className="wall-actions"><button className={`action-button ${mode.endsWith('horizontal') ? 'selected' : ''}`} onClick={() => selectWall('horizontal')} disabled={!canAct || game.remaining.blue === 0}><span className="action-icon wall-icon horizontal-icon" /><span><strong>Wall</strong><small>Horizontal</small></span></button><button className={`action-button ${mode.endsWith('vertical') ? 'selected' : ''}`} onClick={() => selectWall('vertical')} disabled={!canAct || game.remaining.blue === 0}><span className="action-icon wall-icon vertical-icon" /><span><strong>Wall</strong><small>Vertical</small></span></button></div>
        ...
        <div className="control-group difficulty-group"><div className="control-label"><span>RIVAL INTELLIGENCE</span><small>LOCAL AI</small></div><div className="difficulty-switch" role="group">{(['easy', 'normal', 'hard'] as const).map(level => <button key={level} className={difficulty === level ? 'active' : ''} onClick={() => setDifficulty(level)}>{level}</button>)}</div></div><div className="route-readout"><div><small>YOUR ROUTE</small><strong>{blueRoute}<span>steps</span></strong></div><div><small>RIVAL ROUTE</small><strong>{redRoute}<span>steps</span></strong></div></div><button className="restart-bottom" onClick={rematch}><span>↺</span> Rematch · {view.rush?.seedLocked ? 'same seed' : rushMode ? 'new seed' : 'same rules'} <span>↗</span></button><button className="change-mode-bottom" onClick={() => setStarted(false)}>Choose another mode, map, or layout</button>
      </aside>
