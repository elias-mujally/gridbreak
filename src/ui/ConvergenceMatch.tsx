...
import { Action, GameState, Point, Wall, activePlayerStates, currentPlayerId, samePoint } from '../game/state';
...
export default function ConvergenceMatch({ game, onAction, onRematch, onMainMenu }: Props) {
    ...
    <main className="main-layout convergence-layout">
      ...
      <section className="arena convergence-arena" aria-label={`${game.width} by ${game.height} Convergence board`}>
        ...
        <div className="mobile-toolbar convergence-toolbar" role="group" aria-label="Current player action">
          <button className={mode === 'move' ? 'selected' : ''} onClick={() => setMode('move')} disabled={!!game.winner}>✣ <span>Move</span></button>
          <button className={mode === 'horizontal' ? 'selected' : ''} onClick={() => setMode('horizontal')} disabled={!!game.winner || active.wallsRemaining === 0}>━ <span>Wall —</span></button>
          <button className={mode === 'vertical' ? 'selected' : ''} onClick={() => setMode('vertical')} disabled={!!game.winner || active.wallsRemaining === 0}>┃ <span>Wall |</span></button>
        </div>
        ...
        <div className={`board-viewport ${boardZoom ? 'zoomed' : ''}`}>
          <div className="board-frame convergence-frame">
            <div className={`board ${game.winner ? 'winner-blue' : ''}`} style={boardStyle} data-mode={mode}>
              ...
              <svg className={`wall-layer ${placementMode && !game.winner ? 'active' : ''}`} viewBox={`0 0 ${boardWidth} ${boardHeight}`} aria-label="Wall placement points">
                ...
                {placementMode && !game.winner ? anchors.map(anchor => {
              ...
              {game.winner ? <div className="match-result convergence-result victory" role="dialog" aria-modal="true" aria-label="Match result">
                ...
                <div className="result-actions"><button onClick={rematch}>↺ REMATCH</button><button className="result-menu" onClick={onMainMenu}>MAIN MENU</button></div>
              </div> : null}
            </div>
          </div>
        </div>
      ...
      <aside className="control-panel convergence-controls">
        <div className="panel-heading"><span className="panel-index">02 / SHARED CONTROL</span><span className="live-pip">LOCAL</span></div>
        ...
        <div className="control-group actions-group"><div className="control-label"><span>ACTION</span><small>ONE PER TURN</small></div>
          <button className={`action-button ${mode === 'move' ? 'selected' : ''}`} onClick={() => setMode('move')} disabled={!!game.winner}><span className="action-icon move-icon">✣</span><span><strong>Move</strong><small>Advance toward center</small></span><span className="action-check">{mode === 'move' ? '●' : '○'}</span></button>
          <div className="wall-actions"><button className={`action-button ${mode === 'horizontal' ? 'selected' : ''}`} onClick={() => setMode('horizontal')} disabled={!!game.winner || active.wallsRemaining === 0}><span className="action-icon wall-icon horizontal-icon" /><span><strong>Wall</strong><small>Horizontal</small></span></button><button className={`action-button ${mode === 'vertical' ? 'selected' : ''}`} onClick={() => setMode('vertical')} disabled={!!game.winner || active.wallsRemaining === 0}><span className="action-icon wall-icon vertical-icon" /><span><strong>Wall</strong><small>Vertical</small></span></button></div>
        </div>
        ...
        <button className="restart-bottom" onClick={rematch}><span>↺</span> Rematch · same setup <span>↗</span></button>
