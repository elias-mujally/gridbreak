import type { GameMode, Player, Point } from './state';
...
export const MAP_CONFIGS: Record<MapId, MapConfig> = {
  ...
  arena: { id: 'arena', name: 'Arena', width: 10, height: 10, startingWalls: 10, layouts: layouts(10, 10), defaultLayout: 'opposite', rewardRange: [4, 6], rewardWeights: { energy: 55, boost: 45 }, suddenDeathPly: 50, deadlinePly: 96, wallDrainEvery: 5, aiWallLimit: 96 },
  ...
  grand: { id: 'grand', name: 'Grand', width: 15, height: 15, startingWalls: 16, layouts: layouts(15, 15), defaultLayout: 'opposite', rewardRange: [8, 10], rewardWeights: { energy: 60, boost: 40 }, suddenDeathPly: 84, deadlinePly: 160, wallDrainEvery: 7, aiWallLimit: 140 },
  titan: { id: 'titan', name: 'Titan', width: 20, height: 20, startingWalls: 20, layouts: layouts(20, 20), defaultLayout: 'opposite', rewardRange: [10, 14], rewardWeights: { energy: 65, boost: 35 }, suddenDeathPly: 120, deadlinePly: 220, wallDrainEvery: 8, aiWallLimit: 160 },
};
...
export const RULE_SETS: Record<GameMode, RuleSet> = {
...
export function layoutConfig(map: MapConfig, id: RaceLayout = map.defaultLayout): LayoutConfig {
  return map.layouts[id] ?? map.layouts[map.defaultLayout]!;
}
export function compatibleLayouts(map: MapConfig): RaceLayout[] {
  return (Object.keys(map.layouts) as RaceLayout[]).filter(id => !!map.layouts[id]);
}
...
export function goalLabel(goal: GoalZone): string { return `${goal.edge.toUpperCase()} EDGE`; }
